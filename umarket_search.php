...
<title>UMARKET - Результаты поиска "<?php echo $query; ?>"</title>
   ...
   <!--Каталог--> 
    <div id="catalog">
        <div class="search-column" id="right-column ">
            ...
            $query_count = "SELECT count(`sub_id`) FROM `catalog` WHERE (`name` LIKE '%".$query."%' OR `description` LIKE '%".$query."%' OR `article` LIKE '%".$query."%' OR `price` LIKE '%".$query."%') AND sub_id BETWEEN 40 AND 51";
			...
			$sql = mysql_query("SELECT * FROM `catalog` WHERE (`name` LIKE '%".$query."%' OR `description` LIKE '%".$query."%' OR `article` LIKE '%".$query."%' OR `price` LIKE '%".$query."%') AND sub_id BETWEEN 40 AND 51 ORDER BY `id` DESC LIMIT $start, $num");
	 ...
	 <!-- Кнопка загрузить еще -->
			...
			if ( $posts > 25 && $_GET['page'] != $total) {
			  ...
			  if ($page != $total) {
				...
				echo '<a href="umarket_search.php?q='.$query.'&page='. ($page + 1)  .'" class="btn btn-load-more" id="loadMore">Загрузить ещё</a>';
			  }
			}
			?>
			</div>
		...
		<a href="umarket" class="btn btn-back" id="back">Назад в каталог</a>
