...
//$pagination->url   = 'catalog-mobile.php' . $url . '&page={page}';
...
if (!empty($catid)) {
    $urlc = get_url('catalog-mobile.php' . $url);
}
if (!empty($subid)) {
    $urlc = get_url('catalog-mobile.php' . $url);
}
...
<div class="browserupgrade">Вы используете <strong>устаревшую</strong> версию браузера.Пожалуйста <a
		href="http://browsehappy.com/">обновите свой браузер</a>.</div>
<![endif] -->
	...
	<section class="catalog">
		<div class="container">
			...
			<div class="block-return"><img src="/mobile/img/icon/arrow_back.svg" alt="Back" />
				<a class="return" href="#" onclick="window.history.back(); return false;" >назад</a>
			</div>
		</div>
        ...
        <?php if (isset($_GET['status']) != 3) { ?>
			<div class="container">
				<div class="block-filter">
					<a class="search-filter" href="filter-mobile.php?<?php if (isset($_GET['catid'])) {
                        echo 'catid=' . $catid . '';
                    } elseif (isset($_GET['subid']) == 25) {
                        echo 'subid=' . $subid . '';
                    } ?>">Поиск по категории</a>
				</div>
			</div>
        <?php } ?>
		<div class="container">
                            ...
                            foreach ($rows as $row) {
								...
								<li>
									<div <?php if ($row['status'] == 3) {
										...
										<a href="product-mobile.php?catid=<?= $row['cat_id']; ?>&item=<?= $row['id'] ?>#anchor" name="anchor">
											<div class="view">
												<div class="status
                                                <?php
                                                if ($row['status'] == '0') {
                                                ...
                                                <?php if ($row['catalog_image']) {
                                                    echo '<img class="lazy" data-src="../../uploads/catalog/' . $row['catalog_image'] . '" alt="' . $row['description'] . '">';
                                                } ?>
											</div>
			...
			<div class="block-sticky">
				...
				<div class="block-delivery">
					<p class="delivery-title">
						Бесплатная доставка от € 100</p>
					<img class="close-delivery" src="/mobile/img/icon/button_close.svg" alt="Close"
						 id="CloseDelivery" />
				</div>
			</div>
		</div>
