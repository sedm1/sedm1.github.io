...
<body>
	...
	<div id="wrapper">
		...
		<div id="catalog">
						...
						foreach ($arr2 as $cat) {
							...
							<li <?php if ($cat['id'] == $_SESSION['catid']) : ?> class="expand" <?php endif; ?>>
								<?php if (isset($cat['id']) && $cat['id'] == 3) { ?>
									<a href="?subid=25"><?= $cat['category_en'] ?></a>
								<?php } else { ?>
									<a href="?catid=<?= $cat['id'] ?>"><?= $cat['category_en'] ?></a>
								<?php } ?>
								<ul class="acitem">
									<?php
													...
													<ul <?php if (isset($_GET['subid']) && $_GET['subid'] == $sub["id"]) : ?> style="display:block; top:8px;" class="clearfix" <?php else : ?> style="display:none;">
														...
														<li class="filter-slider clearfix">
															<?php if (!empty($sort_age)) : ?>
																<?php
																$ages = array(
																	1 => 'from 0 till 12 months',
																	2 => 'from 1 year till 5 years',
																	3 => 'from 5 till 10 years',
																	4 => 'from 10 till 15 years',
																);
																ksort($ages);
																?>
								...
								?>
									<?php if (isset($cat['id']) && $cat['id'] != 3) { ?>
										<li class="all-products-link"><a href="?catid=<?= $cat['id'] ?>">All goods</a></li>
									<?php } ?>
								</ul>
							</li>
			...
			<!--Доставка и оплата-->
			<div id="delivery_en"></div>

			<div id="message">
				<div id="left-column"></div>
				<div id="right-column">
					Dear visitors ! Unfortunately, we not always have a time to prepare and load on the website a full goods specification. If you like some products from our assortment please write us by email and we will provide you all need information.
				</div>
			</div>
		</div>
        <!--Кнопка вверх-->
