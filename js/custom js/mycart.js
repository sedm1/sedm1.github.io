...
$(document).ready(function () {
    ...
    $('.sl').click(function () {
        var id = $(this).data('id');
        if (id == 'ru') {
            var hreff = "/checkout.php";
        } else {
            var hrefff = "/checkout_" + id + ".php";
        }

        var href = "/checkout_en.php";
    ...
    options.change(function () {
        ...
        $('#small-photo li').css('height', '174px')
    });
    ...
    $(document).on('click', '.addToCart', function () {
        ...
        if (options !== '') {
            ...
            $('#small-photo li').css('height', '150px')
        }
            ...
            $.ajax({
                ...
                success: function (json) {
                    if (json['success'].length) {
                        $('#bg_layer').append('<div id="modal-success">' + json['success'] + '</div>');
                        $('#bg_layer').show();
                        setTimeout(function () {
                            /*location.reload();*/
...
var mycart = {
    'add': function (key, quantity, options) {
        $.ajax({
            url: '/cart.php?add',
            type: 'get',
            data: 'id=' + key + '&quantity=' + (typeof (quantity) != 'undefined' ? quantity : 1) + '&options=' + options,
            dataType: 'json',
            success: function (json) {
    ...
    'update': function (key, quantity, options, js) {
        $.ajax({
            ...
            success: function (json) {
                ...
                var ptotal = Number.parseInt($('#price' + key + '-' + js).text()) * quantity;
