...
$where = addWhere($where, "`sub_id` between 40 and 50 ");
...
// $pagination->url = 'umarket_lv.php' . $url . '&page={page}';
...
<head>
  ...
  <title>UMARKET - Interneta veikals</title>
   ...
   <div class="layer"></div>
    ...
    <div id="umarket-banner">
            <div class="umarket-banner-wrap">
                <h2>Mazlietoti zīmolu preces bez defektiem.</h2>
                <p>Viss vienā vietā. Piegāde visā Latvijā.</p>
            </div>
    </div>
    <!-- Каталог -->
    <div id="catalog" style="position: relative;">

      <h2 class="catalog__title">Preču katalogs</h2>
      <!-- Первый фильтр -->
      <div class="filter">
        <ul>
        <li><a href="<?php if (!empty($catid) || !empty($subid)){echo 'umarket_lv?page=1';}?>"
          <?php if (empty($catid) && empty($subid)) {
                                    echo 'id="active"';
                                  } ?> >Visi</a></li>

          <li><a href="<?php echo "?catid=7&page=1"?>"<?php if ($catid == 7 ) {
                                    echo 'id="active-first"';
                                  } ?>>Apģērbi</a></li>
          <li><a href="<?php echo "?catid=8&page=1"?>" <?php if ($catid == 8 ) {
                                    echo 'id="active-first"';
                                  } ?>>Apavi</a></li>
          <li><a href="<?php echo "?catid=9&page=1"?>" <?php if ($catid == 9) {
                                    echo 'id="active-first"';
                                  } ?>>Aksesuāri</a></li>
          <li><a href="<?php echo "?catid=1&page=10"?>" <?php if ($catid == 10) {
                                    echo 'id="active-first"';
                                  } ?>>Elektropreces</a></li>
          <li><a href="<?php echo "?catid=11&page=1"?>" <?php if ($catid == 11) {
                                    echo 'id="active-first"';
                                  } ?>>Citas preces</a></li>
        </ul>
      </div>
      <!-- Фильтр -->
      <?php if ($_GET['catid'] =='7' || $_GET['catid'] =='8' || isset($_GET['subid'])) {;?>
      <div id="filter" style="display:block;">
      <?php }else{
       echo "<div id='filter' style='display:none;'>";} ?>
      
        <ul>
        <li><a href="<?php if($catid == 7){echo "?catid=7&subid=40&page=1";
          }elseif($catid == 8 )
         ...
         <?php if ($catid == 7 && $subid == 40 || $catid == 8 && $subid == 43 || $catid == 9 && $subid == 46 ) {echo 'id="active"';} ?>>Sieviešu</a></li>




        <li><a href="<?php if($catid== 7){echo "?catid=7&subid=41&page=1";
          }elseif($catid == 8)
          ...
          // elseif($catid == 11)
          // {echo "?catid=11&subid=50";} ?>"
          <?php if ($catid == 7 && $subid == 41 || $catid == 8 && $subid == 44 || $catid == 9 && $subid == 47) {echo 'id="active"';}?>>Vīriešu </a></li>




        <li><a href="<?php if($catid == 7){ echo "?catid=7&subid=42&page=1";
          }elseif($catid == 8 )
          ...
          // elseif($catid == 11)
          // {echo "?catid=11&subid=50";}?>"
          <?php if ($catid == 7 && $subid == 42 || $catid == 8 && $subid == 45 || $catid == 9 && $subid == 48) {echo 'id="active"';}?>
          >Bērnu</a></li>
        </ul>
      </div>
      <!--Список товаров-->
      <div>
        ...
        <div id="products-list">
          <div id="item-list">
            <ul>
              <?php
              //Запрос для вывода umarket подкатегорий
              if (count($row_BU) > 0) {
                ...
                foreach ($row_BU as $row) {
                  ...
                  <li>
                    <div <?php if ($row['status'] == 3) {
                      ...
                      <a href="umarket_view_lv?item=<?= $row['id'] ?>">
                        <div class="view">
                                <div class="status 
                                  <?php
                ...
                echo '<div class="empty-product">Piedāvājumi Nav</div>';
              }
              ?>
            </ul>
          </div>
       ...
       <?php 
            ...
            if ($total > $num  && $_GET['page'] != $num_pages ) {
            
              
              if (!empty($catid)  && !empty($subid)) {
                  
                echo '<a href="'.$_SERVER['PHP_SELF'].'?catid='.$catid.'&subid='.$subid. '&page='.$cur_page.'" class="btn btn-load-more" id="loadMore">Rādīt vēl</a>';

            } elseif (!empty($catid)) {

                echo '<a href="'.$_SERVER['PHP_SELF'].'?catid='.$catid.'&page='.$cur_page.'" class="btn btn-load-more" id="loadMore">Rādīt vēl</a>';

              } else {

                echo '<a href="'.$_SERVER['PHP_SELF'].'?page='.$cur_page.'" class="btn btn-load-more" id="loadMore">Rādīt vēl</a>';
              }
            }
          
         
          
            // echo '<pre> Кол-во товаров на странице<br>';
            // var_dump($num);
            // echo '</pre> <br>';
            // // echo '<pre>';
            // // var_dump($count_bu['count']);
            // // echo '</pre> <br>';
            // echo '<pre> Текущая страница<br>';
            // var_dump($page);
            // echo '</pre> <br>';
            // echo '<pre> Всего страниц<br>';
            // var_dump($num_pages);
            // echo '</pre> <br>';
            // echo '<pre> Всего товаров<br>';
            // var_dump($total);
            // echo '</pre>'
            // ?>
        ...
        <aside class="rigth-aside">
          ...
          <div class="popup-container popup-container-email--rectangle" id="popup-email">
              <div class="popup-heading">
                <h5>Uzrakstīt mums</h5>
                <a class="close-modal-email" href="#"><img src="/images/umarket_close-modal.png" alt="close" /></a>
              </div>
              <div class="form-block">
                <form class="form" action="http://<?php echo $_SERVER['HTTP_HOST'] . '/umarket_send_mail.php?lang=lv';?>"method="POST">
                  <input type="email" name="email" class="modal-email" placeholder="Email"  required/>
                  <textarea name="message" placeholder="Jūsu ziņojums" cols="30" rows="10" required></textarea>
                  <div class="captch-block"><img src="umarket_captcha.php?color=#023BC7" alt="Img captcha"/>
                    <input type="number" name="kapcha" placeholder="captcha" required />
                    <input type="hidden" name="captcha_key" value="" />
                  </div>
                  <button class="btn-modal-right" type="submit" id="btn-email">Sūtīt</button>
                </form>
              </div>
          </div>
        </aside>
