...
require_once("library/pagination_mob.php"); //Подключение пагинацию
...
// Получаем настройки
$col_list_mobile = mysql_query("SELECT `col_list_mobile` FROM userlist");
$col             = mysql_fetch_array($col_list_mobile);
$num             = $col['col_list_mobile']; // Количество записей на странице
...
//$pagination        = new Pagination_mob();
//$pagination->total = $total;
//$pagination->page  = $page;
//$pagination->limit = $num;
//$pagination->url   = 'catalog-filter-mobile.php' . $url . '&page={page}';
...
if (!empty($catid)) {
    $urlc = get_url('catalog-filter-mobile.php' . $url);
}
if (!empty($subid)) {
    $urlc = get_url('catalog-filter-mobile.php' . $url);
}
	...
	<script>
        $(function () {
            ...
            $('.filter-form-cats').on('change', function () {
                ...
                $('#products-list').html('<div class="products-loading" style="display:flex; align-items:center; justify-content:center;"><img src="<?php echo get_url('images/spin.gif'); ?>"></div>');
                $.post(url, function (html) {
					...
					// 	'<a class="return" href="#" onclick="window.history.go(-3); return false;" >назад</a>');
...
<body>
<!-- [if lt IE 8]>
<div class="browserupgrade">Вы используете <strong>устаревшую</strong> версию браузера.Пожалуйста <a
		href="http://browsehappy.com/">обновите свой браузер</a>.
</div>
<![endif]-->
<div id="bg_layer"></div>
	...
	<section class="catalog catalog-size">
		<div class="container">
			...
			<div class="block-return"><img src="/mobile/img/icon/arrow_back.svg" alt="Back" />
				<a class="return" href="#" onclick="window.history.go(-2); return false;" >назад</a>
            </div>
		</div>
		<!--Блок поиск по размерам-->
		<div class="block-filter"><span class="search-filter">Поиск по размерам</span>
		</div>
		<div class="container">
			<!--Блок выбор размера-->
                                ...
                                foreach ($rows as $row) {
									...
									<li>
										<div <?php if ($row['status'] == 3) {
											...
											<a href="product-mobile.php?item=<?= $row['id'] ?>#<?php echo $row['id']; ?>"
											   name="<?php echo $row['id']; ?>">
                                                ...
                                                <?php
                                                    ...
                                                    <?php if ($row['catalog_image']) {
                                                        echo '<img class="lazy" src="../../uploads/catalog/' . $row['catalog_image'] . '" alt="' . $row['description'] . '">';
                                                    } ?>
												</div>
				...
				<div class="block-sticky">
					...
					<div class="block-delivery">
						<p class="delivery-title">
							Бесплатная доставка от € 100</p><img class="close-delivery"
