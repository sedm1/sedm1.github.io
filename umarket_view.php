...
<div id="wrapper">
    ...
    <div id="umarket-banner-product">
		...
		<p>Все товары можно посмотреть и приобрести на месте с 9.00 до 21.00.</p> <p>Наш адрес - Slokas Iela 52 (LV-1007) Riga.</p> <p>Перед тем как ехать к нам, обязательно согласуйте время.</p> <p>Также возможна отправка по всей Латвии через Omniva или по почте.</p>
		</div>
	</div>
  	<!--Товар-->
    <div id="product">
            ...
            <div id="right-column">
                ...
                */?>
				...
				<!-- style="top: 125px; position:relative;" -->
                    ...
                    <span id="article" <?php if ($item['status'] == '2') { echo 'style="margin:35px 0 22px 0;"' ;}?>><?=$item['article'] ?></span>
                    ...
                    <span id="order-button" <?php if ($item['status'] == '2') { echo "class=\"order-button-sold\""; }?>>
						<a style="background-color: #B1B0B0;color: #333333;" <?php if ($item['status'] != '2') { ?> href="#ex1" rel="modal:open" <?php } ?> >Купить сразу</a>
						<?php if ($item['status'] != '2') : ?>
						  <a class="addToCart" href="javascript:void(0)" data-id="<?php echo $item_id; ?>">В корзину</a>
						<?php endif; ?>
					</span>
        ...
        <div id="item-list">
            ...
            <ul>
            	<?php
				$rand_product = mysql_query("SELECT `id`, `cat_id`, `name`, `description`, `text`, `article`, `price`, `catalog_image`, `status`,`price_stock` FROM `catalog` WHERE status !='2' and `id` !='".$item_id."' and `cat_id` BETWEEN 7 AND 12 ORDER BY RAND() LIMIT 12");
                ...
                <li <?php if($offer['status'] == 3){ echo 'style="box-shadow: 0px 0 0px 3px red;"'; }?>>
                    <a href="umarket_view?item=<?=$offer['id'] ?>">
        ...
        <a onclick="window.history.go(-1); return false;" class="btn btn-back" id="back">Назад в каталог</a>
	...
	<script type="text/javascript">
	 ...
	 <div id="umarket-banner-under">
            <div class="umarket-banner-wrap">
                <p>Новые брендовые товары по низкой цене. Регулярные поставки с магазинов. Доставка по всей Латвии.</p>
            </div>
