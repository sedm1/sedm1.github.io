...
<div id="item-list">
  ...
  <ul>
  <?php
	$rand_product = mysql_query("SELECT * FROM `catalog` WHERE `cat_id` NOT IN (7,8,9,10,11,12) AND `status` NOT IN (2) ORDER BY RAND() LIMIT 5");
  ...
  <li>
	...
	<a href="view<?php echo $ext; ?>.php?item=<?=$offer['id'] ?>">
	<div class="view">
	  <div class="status 
	  <?php 
		if ($offer['status'] == '0') {
