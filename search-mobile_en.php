...
//if ($page != 1) $pervpage = '<a href="search-mobile_en.php?q=' . $query . '&page=-1">First</a>
//			<div class="nav_arrow_left"><a href="search-mobile_en.php?q=' . $query . '&page=' . ($page - 1) . '"></a></div> ';
//
//if ($page != $total) $nextpage = '  <div class="nav_arrow_right"><a href="search-mobile_en.php?q=' . $query . '&page=' . ($page + 1) . '"></a></div>
//			<a href="search-mobile_en.php?q=' . $query . '&page=' . $total . '">Last</a> ';
//
//if ($page - 2 > 0) $page2left = ' <a href="search-mobile_en.php?q=' . $query . '&page=' . ($page - 2) . '">' . ($page - 2) . '</a>  ';
//if ($page - 1 > 0) $page1left = '<a href="search-mobile_en.php?q=' . $query . '&page=' . ($page - 1) . '">' . ($page - 1) . '</a>  ';
//if ($page + 2 <= $total) $page2right = '  <a href="search-mobile_en.php?q=' . $query . '&page=' . ($page + 2) . '">' . ($page + 2) . '</a>';
//if ($page + 1 <= $total) $page1right = '  <a href="search-mobile_en.php?q=' . $query . '&page=' . ($page + 1) . '">' . ($page + 1) . '</a>';


$sql = mysql_query("SELECT * FROM `catalog` WHERE (`name` LIKE '%" . $query . "%' OR `description` LIKE '%" . $query . "%' OR `article` LIKE '%" . $query . "%' OR `price` LIKE '%" . $query . "%') AND (status!=2) AND  `sub_id`  NOT BETWEEN 40 AND 51 ORDER BY `id` DESC");
...
<div class="catalog">
	<div class="container">
		...
		<div class="block-return"><img src="/mobile/img/icon/arrow_back.svg" alt="Back" />
			<a class="return" href="#" onclick="window.history.go(-1); return false;">back</a>
		</div>
	</div>
	<!--//Блок Назад-->
	<div class="container">
		...
		<div class="search-result-text">
			<p class="result-search">Searching results "<?php echo $query; ?>"</p>
			<p class="products-result">Found <?php echo $posts; ?> of goods </p>
		</div>
		...
		<div class="item-list" id="item-list">
			<ul>
                ...
                if (!empty($row['id'])) {
                    ...
                    echo '<div class="empty-product">no offers</div>';
                }
                ?>
