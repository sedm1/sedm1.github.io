...
require_once("library/pagination_mob_en.php"); //Подключение пагинацию
...
/*$pagination = new Pagination_mob_en();
$pagination->total = $total;
$pagination->page = $page;
$pagination->limit = $num;
$pagination->url = 'catalog-filter-mobile_en.php' . $url . '&page={page}';
...
if (!empty($catid)) {
    $urlc = get_url('catalog-filter-mobile_en.php' . $url);
}
if (!empty($subid)) {
    $urlc = get_url('catalog-filter-mobile_en.php' . $url);
}
            ...
            $('.filter-form-cats').on('change', function () {
                ...
                $.get(url, function (html) {
                    ...
                    $('.block-return').html('<img src="/mobile/img/icon/arrow_back.svg" alt="Back" />' +
                        '<a class="return" href="#" onclick="window.history.go(-3); return false;" >back</a>');
                });
	...
	<section class="catalog catalog-size">
		<div class="container">
			...
			<div class="block-return"><img src="/mobile/img/icon/arrow_back.svg" alt="Back" />
				<a class="return" href="#" onclick="window.history.go(-2); return false;" >back</a>
			</div>
		</div>
		<!--Блок поиск по размерам-->
		<div class="block-filter"><span class="search-filter">Search per Filter</span>
		</div>
		<div class="container">
			<!--Блок выбор размера-->
                                ...
                                foreach ($rows as $row) {
									...
									<li>
										<div <?php if ($row['status'] == 3) {
											...
											<a href="product-mobile_en.php?item=<?= $row['id'] ?>">
												<div class="view">
													<div class="status
                                                <?php
                                                    if ($row['status'] == '0') {
                                                    ...
                                                    <?php if ($row['catalog_image']) {
                                                        echo '<img src="../../uploads/catalog/' . $row['catalog_image'] . '" alt="' . $row['description_en'] . '">';
                                                    } ?>
												</div>
				...
				<div class="block-sticky">
					...
					<div class="block-delivery">
						<p class="delivery-title">
							FREE SHIPPING FROM € 100</p><img class="close-delivery"
