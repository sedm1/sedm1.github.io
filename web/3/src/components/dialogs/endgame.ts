export const createEndGameDialog = (): HTMLDialogElement => {
    let endGameDialog = document.querySelector('dialog.endGame') as HTMLDialogElement | null;
    if (!endGameDialog) {
...
export const dialogOpen = (root: Element, score: number) => {
    ...
    let startNewGameButton = endGameDialog.querySelector('.endGame-newGame') as HTMLButtonElement | null;
    ...
    let setRecordButton = endGameDialog.querySelector('.endGame-setRecordButton') as HTMLButtonElement | null;
    let recordInputContainer = endGameDialog.querySelector('.endGame-recordContainer') as HTMLDivElement | null;
    ...
    if (!setRecordButton) {
        ...
        setRecordButton.addEventListener('click', () => {
            if (recordInputContainer) {
                const inp = recordInputContainer.querySelector('input') as HTMLInputElement | null;
                inp?.focus();
                return;
            }
            ...
            confirmBtn.addEventListener('click', () => {
                ...
                if (setRecordButton) setRecordButton.style.display = 'none';
